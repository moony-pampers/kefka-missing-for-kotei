...
function markSide(player, round) {
  ...
  // 左右担当: 塔1（左）寄りの順 H1, H2, MT, ST, D1, D2, D3, D4（右寄り）
  // 被り時は「本来の側に近いロールが残り、遠いロールが反対へ移動」
  // sharedTower===0(塔1/左) なら H寄りが残る → indexが小さい方が残る
  // sharedTower===1(塔2/右) なら D寄りが残る → indexが大きい方が残る
  const TOWER_SIDE_ORDER = ["H1", "H2", "MT", "ST", "D1", "D2", "D3", "D4"];
  const indexA = TOWER_SIDE_ORDER.indexOf(peers[0].id);
  const indexB = TOWER_SIDE_ORDER.indexOf(peers[1].id);
  // 塔1(左)なら小さいindexが残る、塔2(右)なら大きいindexが残る
  const stayPeer = sharedTower === 0
    ? (indexA <= indexB ? peers[0] : peers[1])
    : (indexA >= indexB ? peers[0] : peers[1]);
  return stayPeer === player ? sharedTower : 1 - sharedTower;
}
...
function drawMark(player) {
  ...
  if (player.mark === "share") {
    ...
    // 十字方向に4つの菱形を描画（頭割りマーカー）
    const d = 5.5; // 菱形の半径
    const offset = 7; // 中心からの距離
    for (const [ox, oy] of [[0, -offset], [offset, 0], [0, offset], [-offset, 0]]) {
      ctx.beginPath();
      ctx.moveTo(ox, oy - d);
      ctx.lineTo(ox + d, oy);
      ctx.lineTo(ox, oy + d);
...
// 固定設定: りーん式 / はいじあ/DN式 / 前回塔維持
const FIXED_STRATEGY = "lean";
const FIXED_SPREAD = "dn";
const FIXED_TOWER_PRIORITY = "keepPrevious";
